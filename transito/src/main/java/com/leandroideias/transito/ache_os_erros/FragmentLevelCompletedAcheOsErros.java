...
/**
 * Created by Leandro on 18/5/2014.
 */
public class FragmentLevelCompletedAcheOsErros extends DialogFragment implements View.OnClickListener {
	public static final String TAG = "FragmentLevelCompletedAcheOsErros";
	private ItemDatabase itemDatabase;

	private ImageView star1, star2, star3;
	private TextView textViewQuantErros;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		try {
			itemDatabase = (ItemDatabase) getArguments().getSerializable("itemDatabaseAcheOsErros");
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		int layoutResId = (itemDatabase.getQuantErros() < 10 ? R.layout.fragment_completed_level_ache_os_erros : R.layout.fragment_failed_level_ache_os_erros);
	...
	private void initView(View view) {
		boolean failed = (itemDatabase.getQuantErros() >= 10);
		...
		textViewQuantErros = (TextView) view.findViewById(R.id.textViewQuantidadeErrosCompleted);
		...
		textViewQuantErros.setText(getString(R.string.erros_dois_pontos) + " " + itemDatabase.getQuantErros());
		...
		if(itemDatabase.getLevel() == Constants.QUANT_LEVELS_ACHE_OS_ERROS){
	...
	public void onClick(View view) {
		...
		switch(view.getId()){
			...
			case R.id.icNextCompleted:
				activity.recarregarFragment(Math.min(Constants.QUANT_LEVELS_ACHE_OS_ERROS, itemDatabase.getLevel() + 1));
				getDialog().dismiss();
				break;
		}
	}
